...
export class News extends Component {
  ...
  async componentDidMount(){ 
    let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=028c82e1117c4615a152c8c3862e1e4c&pageSize=${this.props.pageSize}&page=1`; //here in class based component no need to pass props as argument becoz classes never takes arg/parameter as funtions
  ...
  handlePrevClick = async ()=>{
    console.log("prev");
      let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=028c82e1117c4615a152c8c3862e1e4c&pageSize=${this.props.pageSize}&page=${this.state.page - 1}`; //setting pageSize varible by giving props to the Newj component
      this.setState({loading: true})
      let data = await fetch(url);
      let parsedData = await data.json();
      this.setState({
  ...
  handleNextClick = async ()=>{
    ...
    if(!(this.state.page + 1 > Math.ceil(this.state.totalResults/this.props.pageSize))){  //21 is replaced by this.props.pageSize everywhere
      ...
      let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=028c82e1117c4615a152c8c3862e1e4c&pageSize=${this.props.pageSize}&page=${this.state.page + 1}`;
      this.setState({loading: true});
      let data = await fetch(url);
      let parsedData = await data.json();
      this.setState({
